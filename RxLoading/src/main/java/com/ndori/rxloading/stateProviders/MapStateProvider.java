...
public class MapStateProvider<T extends Map> extends DefaultStateProvider<T> {

    @Override
    public ILoadingLayout.LoadingState nextState(T map) {
        if ( map == null)
            return ILoadingLayout.LoadingState.LOADING;
        if ( map.isEmpty())
            return ILoadingLayout.LoadingState.NO_DATA;
        return super.nextState(map);
    }
}
